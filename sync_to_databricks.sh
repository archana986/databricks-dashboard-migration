...
upload_file() {
    ...
    # For Python files in folders (helpers/, etc.), delete first then upload without overwrite
    # This avoids the "Overwrite cannot be used for source format when importing a folder" error
    if [[ $file == *.py ]] && [[ $file == */* ]]; then
        # Delete if exists (ignore errors)
        databricks --profile "$PROFILE" workspace delete "$remote_path" &> /dev/null || true
